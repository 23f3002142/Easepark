...
export async function downloadReceipt(reservationId: number): Promise<void> {
  const response = await api.get(`/user/receipt/${reservationId}`, { responseType: 'blob' })
  const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }))
  const link = document.createElement('a')
  link.href = url
  link.setAttribute('download', `easepark_receipt_${reservationId}.pdf`)
